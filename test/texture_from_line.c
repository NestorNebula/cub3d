...
int	main(void)
{
	...
	cut_expect(texture_from_line("NO " NO_PATH, &textures) != 0,
			unit_test, "returns 1 after setting NO texture");
	cut_expect(strcmp(textures.no.path, NO_PATH) == 0,
			unit_test, "stores correct path for NO texture");
	cut_expect(texture_from_line("SO " SO_PATH, &textures) != 0,
			unit_test, "returns 1 after setting SO texture");
	cut_expect(strcmp(textures.so.path, SO_PATH) == 0,
			unit_test, "stores correct path for SO texture");
	cut_expect(texture_from_line("WE " WE_PATH, &textures) != 0,
			unit_test, "returns 1 after setting WE texture");
	cut_expect(strcmp(textures.we.path, WE_PATH) == 0,
			unit_test, "stores correct path for WE texture");
	cut_expect(texture_from_line("EA " EA_PATH, &textures) != 0,
			unit_test, "returns 1 after setting EA texture");
	cut_expect(strcmp(textures.ea.path, EA_PATH) == 0,
			unit_test, "stores correct path for EA texture");
	cut_expect(texture_from_line("F " F_R "," F_G "," F_B, &textures) != 0,
			unit_test, "returns 1 after setting F color");
	cut_expect((textures.f >> 16 & 255) == atoi(F_R),
			unit_test, "sets F's red value correctly");
	cut_expect((textures.f >> 8 & 255) == atoi(F_G),
			unit_test, "sets F's green value correctly");
	cut_expect((textures.f & 255) == atoi(F_B),
			unit_test, "sets F's blue value correctly");
	cut_expect(texture_from_line("C " C_R "," C_G "," C_B, &textures) != 0,
			unit_test, "returns 1 after setting C color");
	cut_expect((textures.c >> 16 & 255) == atoi(C_R),
			unit_test, "sets C's red value correctly");
	cut_expect((textures.c >> 8 & 255) == atoi(C_G),
			unit_test, "sets C's green value correctly");
	cut_expect((textures.c & 255) == atoi(C_B),
	...
	cut_expect(texture_from_line("A" " NA", &textures) == 0,
			unit_test, "returns 0 for invalid identifier");
	cut_expect(texture_from_line("NO " "no_such_file", &textures) == 0,
			unit_test, "returns 0 for invalid filepath");
	cut_expect(texture_from_line("F" "255,255,256", &textures) == 0,
			unit_test, "returns 0 for invalid color (overflow)");
	cut_expect(texture_from_line("F" "255,-1,255", &textures) == 0,
			unit_test, "returns 0 for invalid color (negative)");
	cut_expect(texture_from_line("F" "255,255,255,255", &textures) == 0,
			unit_test, "returns 0 for invalid color (longer than RGB format)");
	cut_expect(texture_from_line("F" "255,255", &textures) == 0,
